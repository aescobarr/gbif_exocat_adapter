...
def process_files(download_folder, reverse_resolution_cache):
    logger.info("*********************")
    ...
    for f in files:
        with open(f) as csv_file:
            ...
            for row in csv_reader:
                try:
                    translated_dict = adapter.translate(row)
                    #print("Hit taxon - " + str(translated_dict))
                    if translated_dict['long'] != '' and translated_dict['lat'] != '':
                        database.sql_insert_citacio(translated_dict,'dades_biocat_gbif_' + date_now)
                        taxon_count += 1
                except KeyError:
...
def main():
    ...
    folder_name = "download_20241217_121319"
    ...
    process_files(folder_name, cached_resolution_file)
    # create_species_file()
    #
